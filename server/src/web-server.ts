...
app.post("/download", async (req, res) => {
  try {
    ...
    for (const webtoonId of request.webtoonIds) {
      ...
      if (!webtoon) {
        console.error(`Webtoon not found: ${webtoonId}`);
        continue;
      }
      ...
      if (!existingDownload) {
        ...
        if (useDbQueue) {
          await db.queue.create({
            data: {
              id: downloadId,
              type: "DOWNLOAD",
              payload: { ...request, webtoonId, url: webtoon.url } as any,
        ...
        processDownload(existingDownload.downloadId, {
          ...request,
          webtoonId,
          url: webtoon.url,
        });
...
async function processDownload(downloadId: string, options: DownloadRequest) {
  try {
    ...
    const { data: html } = await axios.get(options.url!);
    ...
    await emailService.queueEmail(
      ...
      "Download Error",
      `An error occurred while downloading webtoon with ID: ${options.webtoonId}. Error: ${error.message}`
    );
  }
}
    ...
    try {
      ...
      if (data.action === "start_download") {
        ...
        try {
          ...
          processDownload(downloadId, request);
        } catch (error) {
          console.error("Download error:", error);
          ws.send(
            JSON.stringify({
